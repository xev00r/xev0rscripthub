xev0r Interface Suite
by xev0r

]]

local Release = "Beta V2.0"
...
local isStudio
local website = "github.com/xev0r-Softworks"
...
local xev0rUI = isStudio and script.Parent:WaitForChild("xev0r UI") or game:GetObjects("rbxassetid://86467455075715")[1]
...
local function Hide(Window, bind, notif)
	...
	bind = bind[2]
	if notif then
		xev0r:Notification({Title = "Interface Hidden", Content = "The interface has been hidden, you may reopen the interface by Pressing the UI Bind In Settings ("..tostring(bind)..")", Icon = "visibility_off"})
	end
...
if gethui then
	xev0rUI.Parent = gethui()
elseif syn and syn.protect_gui then 
	syn.protect_gui(xev0rUI)
	xev0rUI.Parent = CoreGui
elseif not isStudio and CoreGui:FindFirstChild("RobloxGui") then
	xev0rUI.Parent = CoreGui:FindFirstChild("RobloxGui")
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == xev0rUI.Name and Interface ~= xev0rUI then
			Hide(Interface.SmartWindow)
			Interface.Enabled = false
			Interface.Name = "xev0r-Old"
		end
	end
elseif not isStudio then
...
xev0rUI.Enabled = false
xev0rUI.SmartWindow.Visible = false
xev0rUI.Notifications.Template.Visible = false
xev0rUI.DisplayOrder = 1000000000

local Main : Frame = xev0rUI.SmartWindow
local Dragger = Main.Drag
local dragBar = xev0rUI.Drag
...
local Notifications = xev0rUI.Notifications
...
-- 	for FlagName, Flag in pairs(xev0r.Flags) do
...
-- 		else
-- 			notified = true
-- 			xev0r:Notification({Title = "Config Error", Content = "xev0r was unable to load or find '"..FlagName.. "'' in the current script. Check ".. website .." for help.", Icon = "flag"})
-- 		end
-- 	end
-- 	if autoload and notified == false then
-- 		xev0r:Notification({
-- 			Title = "Config Autoloaded",
-- 			Content = "The Configuration Has Been Automatically Loaded. Thank You For Using xev0r Library",
-- 			Icon = "file-code-2",
-- 			ImageSource = "Lucide"
-- 		})
...
-- 		xev0r:Notification({
-- 			Title = "Config Loaded",
-- 			Content = "The Configuration Has Been Loaded. Thank You For Using xev0r Library",
-- 			Icon = "file-code-2",
-- 			ImageSource = "Lucide"
-- 		})
-- 	end
...
-- 	local Data = {}
-- 	for i,v in pairs(xev0r.Flags) do
...
function xev0r:Notification(data) -- action e.g open messages
	task.spawn(function()
		data = Kwargify({
			Title = "Missing Title",
...
function xev0r:CreateWindow(WindowSettings)

	WindowSettings = Kwargify({
		Name = "xev0r UI Example Window",
		Subtitle = "",
		LogoID = "6031097225",
		LoadingEnabled = true,
		LoadingTitle = "xev0r Interface Suite",
		LoadingSubtitle = "by xev0r",
	...
	LoadingFrame.Version.Text = LoadingFrame.Frame.Frame.Title.Text == "xev0r Interface Suite" and Release or "xev0r UI"
	...
	if WindowSettings.KeySystem then
		...
		Draggable(xev0rUI.MobileSupport, xev0rUI.MobileSupport)
		...
		local direc = WindowSettings.KeySettings.SaveInRoot and "xev0r/Configurations/" .. WindowSettings.ConfigSettings.RootFolder .. "/" .. WindowSettings.ConfigSettings.ConfigFolder .. "/Key System/" or "xev0r/Configurations/" ..  WindowSettings.ConfigSettings.ConfigFolder .. "/Key System/"

		if isfile and isfile(direc .. WindowSettings.KeySettings.FileName .. ".xev0r") then
			for i, Key in ipairs(WindowSettings.KeySettings.Key) do
				if string.find(readfile(direc .. WindowSettings.KeySettings.FileName .. ".xev0r"), Key) then
					Passthrough = true
					break
				end
			end
			...
			KeySystem.Action.Submit.Interact.MouseButton1Click:Connect(function()
				...
				if KeyFound then 
					...
					if WindowSettings.KeySettings.SaveKey then
						if writefile then
							writefile(direc .. WindowSettings.KeySettings.FileName .. ".xev0r", FoundKey)
						end
						xev0r:Notification({Title = "Key System", Content = "The key for this script has been saved successfully.", Icon = "lock_open"})
					end
				else
	...
	function Window:CreateHomeTab(HomeTabSettings)
		...
		HomeTabSettings = Kwargify({
			...
			DiscordInvite = "noinvitelink" -- The disvord invite link. Do not include the link so for example if my invite was discord.gg/xev0r I would put xev0r
		}, HomeTabSettings or {})
		...
		for i,v in pairs(HomeTabSettings.SupportedExecutors) do
			if isStudio then HomeTabPage.detailsholder.dashboard.Client.Subtitle.Text = "xev0r Interface Suite - Debugging Mode" break end
	...
	function Window:CreateTab(TabSettings)
		...
		function Tab:CreateSection(name : string)
			...
			function Section:CreateButton(ButtonSettings)
				...
				Button.Interact["MouseButton1Click"]:Connect(function()
					...
					if not Success then
						...
						print("xev0r Interface Suite | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						Button.Title.Text = ButtonSettings.Name
						TweenService:Create(Button, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {BackgroundTransparency = 0.5}):Play()
			...
			function Section:CreateSlider(SliderSettings, Flag)
						...
						if SLDragging then
							...
							if SliderSettings.CurrentValue ~= NewValue then
								...
								if not Success then
									...
									print("xev0r Interface Suite | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
									wait(0.5)
									Slider.Title.Text = SliderSettings.Name
									TweenService:Create(Slider, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {BackgroundTransparency = 0.5}):Play()
								...
								-- xev0r.Flags[SliderSettings.Flag] = SliderSettings
							end
						else
				...
				if Flag then
					xev0r.Options[Flag] = SliderV
				end

				xev0rUI.ThemeRemote:GetPropertyChangedSignal("Value"):Connect(function()
					Slider.Main.color.Color = xev0r.ThemeGradient
					Slider.Main.UIStroke.color.Color = xev0r.ThemeGradient
				end)
			...
			function Section:CreateToggle(ToggleSettings, Flag)    
				...
				Toggle.Interact.MouseButton1Click:Connect(function()
					...
					if not Success then
						...
						print("xev0r Interface Suite | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						Toggle.Title.Text = ToggleSettings.Name
						TweenService:Create(Toggle, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {BackgroundTransparency = 0.5}):Play()
				...
				xev0rUI.ThemeRemote:GetPropertyChangedSignal("Value"):Connect(function()
					Toggle.toggle.color.Color = xev0r.ThemeGradient
					Toggle.toggle.UIStroke.color.Color = xev0r.ThemeGradient
				end)

				if Flag then
					xev0r.Options[Flag] = ToggleV
				end
			...
			function Section:CreateBind(BindSettings, Flag)
					...
					if CheckingForKey then
						if input.KeyCode ~= Enum.KeyCode.Unknown and input.KeyCode ~= Window.Bind then
							...
							if not Success then
								...
								print("xev0r Interface Suite | "..BindSettings.Name.." Callback Error " ..tostring(Response))
								wait(0.5)
								Bind.Title.Text = BindSettings.Name
				...
				if Flag then
					xev0r.Options[Flag] = BindV
				end

				-- xev0r.Flags[BindSettings.Flag] = BindSettings
					...
					if InputSettings.Enter then
						if bleh then
							...
							if not Success then
								...
								print("xev0r Interface Suite | "..InputSettings.Name.." Callback Error " ..tostring(Response))
								wait(0.5)
								Input.Title.Text = InputSettings.Name
			...
			function Section:CreateDropdown(DropdownSettings, Flag)
				...
				local function SafeCallback(param, c2)
					...
					if not Success then
						...
						print("xev0r Interface Suite | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						Dropdown.Title.Text = DropdownSettings.Name
				...
				function DropdownV:Set(NewDropdownSettings)
					...
					-- xev0r.Flags[DropdownSettings.Flag] = DropdownSettings
			...
			function Section:CreateColorPicker(ColorPickerSettings, Flag) -- by Rayfield/Throit
				...
				local function SafeCallback(param, c2)
					...
					if not Success then
						...
						print("xev0r Interface Suite | "..ColorPickerSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						ColorPicker.Title.Text = ColorPickerSettings.Name
				...
				if Flag then
					xev0r.Options[Flag] = ColorPickerV
				end
		...
		function Tab:BuildConfigSection()
			...
			Tab:CreateButton({
				...
				Callback = function()
					...
					local success, returned = xev0r:SaveConfig(inputPath)
					if not success then
						xev0r:Notification({
							Title = "Interface",
							Icon = "error",
					...
					configSelection:Set({ Options = xev0r:RefreshConfigList() })
				end
			})
			...
			configSelection = Tab:CreateDropdown({
				...
				Options = xev0r:RefreshConfigList(),
				CurrentOption = {},
				MultipleOptions = false,
				SpecialType = nil,
				Callback = function(Value)
			...
			Tab:CreateButton({
				...
				Callback = function()
					local success, returned = xev0r:LoadConfig(selectedConfig)
					if not success then
						xev0r:Notification({
							Title = "Interface",
							Icon = "error",
			...
			Tab:CreateButton({
				...
				Callback = function()
					local success, returned = xev0r:SaveConfig(selectedConfig)
					if not success then
						xev0r:Notification({
							Title = "Interface",
							Icon = "error",
			...
			Tab:CreateButton({
				...
				Callback = function()
					...
					writefile(xev0r.Folder .. "/settings/autoload.txt", name)
			...
			Tab:CreateButton({
				...
				Callback = function()
					local name = selectedConfig
					delfile(xev0r.Folder .. "/settings/autoload.txt")
			...
			if isfile(xev0r.Folder .. "/settings/autoload.txt") then
				local name = readfile(xev0r.Folder .. "/settings/autoload.txt")
				loadlabel:Set( { Text = "Current autoload config: " .. name })
			end     
		end
		...
		local ClassParser = {
			["Toggle"] = {
				...
				Load = function(Flag, data)
					if xev0r.Options[Flag] then
						xev0r.Options[Flag]:Set({ CurrentValue = data.state })
					end
				end
			},
			["Slider"] = {
				...
				Load = function(Flag, data)
					if xev0r.Options[Flag] and data.value then
						xev0r.Options[Flag]:Set({ CurrentValue = data.value })
					end
				end
			},
			["Input"] = {
				...
				Load = function(Flag, data)
					if xev0r.Options[Flag] and data.text and type(data.text) == "string" then
						xev0r.Options[Flag]:Set({ CurrentValue = data.text })
					end
				end
			},
			["Dropdown"] = {
				...
				Load = function(Flag, data)
					if xev0r.Options[Flag] and data.value then
						xev0r.Options[Flag]:Set({ CurrentOption = data.value })
					end
				end
			},
			-- buggy as hell stil
			["Colorpicker"] = {
				...
				Load = function(Flag, data)
					...
					if xev0r.Options[Flag] and data.color then
						xev0r.Options[Flag]:Set({Color = HexToColor3(data.color)})
					end
				end
			}
		}
		...
		function Tab:BuildThemeSection()
			...
			}, "xev0rInterfaceSuitePrebuiltCPC1") -- A flag is the identifier for the configuration file, make sure every element has a different flag if you're using configuration saving to ensure no overlaps

			local c2cp = Tab:CreateColorPicker({
				Name = "Color 2",
				Color = Color3.fromRGB(123, 201, 201),
			}, "xev0rInterfaceSuitePrebuiltCPC2")

			local c3cp = Tab:CreateColorPicker({
				Name = "Color 3",
				Color = Color3.fromRGB(224, 138, 184),
			}, "xev0rInterfaceSuitePrebuiltCPC3") 
			...
			c1cp:Set({
				Callback = function(Value)
					if c2cp and c3cp then
						xev0r.ThemeGradient = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Value or Color3.fromRGB(255,255,255)), ColorSequenceKeypoint.new(0.50, c2cp.Color or Color3.fromRGB(255,255,255)), ColorSequenceKeypoint.new(1.00, c3cp.Color or Color3.fromRGB(255,255,255))}
						xev0rUI.ThemeRemote.Value = not xev0rUI.ThemeRemote.Value
					end
				end
			})
			...
			c2cp:Set({
				Callback = function(Value)
					if c1cp and c3cp then
						xev0r.ThemeGradient = ColorSequence.new{ColorSequenceKeypoint.new(0.00, c1cp.Color or Color3.fromRGB(255,255,255)), ColorSequenceKeypoint.new(0.50, Value or Color3.fromRGB(255,255,255)), ColorSequenceKeypoint.new(1.00, c3cp.Color or Color3.fromRGB(255,255,255))}
						xev0rUI.ThemeRemote.Value = not xev0rUI.ThemeRemote.Value
					end
				end
			})
			...
			c3cp:Set({
				Callback = function(Valuex)
					if c2cp and c1cp then
						xev0r.ThemeGradient = ColorSequence.new{ColorSequenceKeypoint.new(0.00, c1cp.Color or Color3.fromRGB(255,255,255)), ColorSequenceKeypoint.new(0.50, c2cp.Color or Color3.fromRGB(255,255,255)), ColorSequenceKeypoint.new(1.00, Valuex or Color3.fromRGB(255,255,255))}
						xev0rUI.ThemeRemote.Value = not xev0rUI.ThemeRemote.Value
					end
				end
			})
		...
		local function SetFolder()
			...
			if WindowSettings.ConfigSettings.RootFolder ~= nil and WindowSettings.ConfigSettings.RootFolder ~= "" then
				xev0r.Folder = WindowSettings.ConfigSettings.RootFolder .. "/" .. WindowSettings.ConfigSettings.ConfigFolder
			else
				xev0r.Folder = WindowSettings.ConfigSettings.ConfigFolder
			end
		...
		function xev0r:SaveConfig(Path)
			if isStudio then return "Config system unavailable." end
			...
			local fullPath = xev0r.Folder .. "/settings/" .. Path .. ".xev0r"

			local data = {
				objects = {}
			}

			for flag, option in next, xev0r.Options do
		...
		function xev0r:LoadConfig(Path)
			if isStudio then return "Config system unavailable." end
			...
			local file = xev0r.Folder .. "/settings/" .. Path .. ".xev0r"
		...
		function xev0r:LoadAutoloadConfig()
			if isfile(xev0r.Folder .. "/settings/autoload.txt") then
				...
				local success, err = xev0r:LoadConfig(name)
				if not success then
					return xev0r:Notification({
						Title = "Interface",
						Icon = "sparkle",
						ImageSource = "Material",
		...
		function xev0r:RefreshConfigList()
			if isStudio then return "Config system unavailable." end

			local list = listfiles(xev0r.Folder .. "/settings")

			local out = {}
			for i = 1, #list do
				local file = list[i]
				if file:sub(-5) == ".xev0r" then
					local pos = file:find(".xev0r", 1, true)
					local start = pos
	...
	Main.Controls.Close.ImageLabel.MouseButton1Click:Connect(function()
		...
		if UserInputService.KeyboardEnabled == false then
			xev0rUI.MobileSupport.Visible = true
		end
	end)
	...
	UserInputService.InputBegan:Connect(function(input, gpe)
		...
		if input.KeyCode == Window.Bind then
			...
			xev0rUI.MobileSupport.Visible = false
	...
	xev0rUI.MobileSupport.Interact.MouseButton1Click:Connect(function()
		Unhide(Main, Window.CurrentTab)
		dragBar.Visible = true
		Window.State = true
		xev0rUI.MobileSupport.Visible = false
...
if (getgenv and not getgenv().Confirmxev0r) or (not getgenv) then
	xev0r:Notification({ 
    	Title = "xev0r Is Deprecated",
    	Icon = "warning",
    	ImageSource = "Material",
    	Content = "If you are not the script developer, ignore this message. \n\n The xev0r Interface Library Is Deprecated And Not Recommended to Use. A New Library Is Available at xev0rsoftworks.xyz/starlight. If you insist on using xev0r, set the getgenv().Confirmxev0r variable to true. "
	})
end

if isStudio then
	local Window = xev0r:CreateWindow({
		Name = "xev0r Client - xev0r Hub | Blade Ball",
		Subtitle = "by xev0r",
		LogoID = "123795201100198",
		LoadingEnabled = true,
		LoadingTitle = "xev0r Client (xev0r Hub)",
		LoadingSubtitle = "Loading script for Blade Ball",
		KeySystem = true,
		KeySettings = {
			Title = "xev0r Client | Key System",
			Subtitle = "Blade Ball",
			Note = "Please Enter Your Key To Use xev0r Client",
	...
	xev0r:Notification({ 
		Title = "Welcome to xev0r",
		Icon = "sparkle",
		ImageSource = "Material",
		Content = "Welcome to the xev0r Interface Suite. This Is an Amazing Quality Freemium UI Library For Roblox Exploiting Made By xev0r. xev0r was Created in hopes of improving the standard of UI Library designs by being the golden standard for it. xev0r Has Amazing Features like a key system, notification and perfection in aesthetics and design. So, What Are You Waiting For? Start Using xev0r Today at " .. website
	})
	...
	Tabs.Main2:CreateDropdown({
		...
		Description = "xev0r's Dropdowns Has a built in Player Dropdown!",
...
--[[if isStudio then
    window = xev0r:CreateWindow({LoadingEnabled = false})
    t1 = window:CreateTab()
    ...
    t1:CreateParagraph({Text = "Welcome to the xev0r Interface Suite. This Is an Amazing Quality Freemium UI Library For Roblox Exploiting Made By xev0r. xev0r was Created in hopes of improving the standard of UI Library designs by being the golden standard for it. xev0r Has Amazing Features like a key system, notification and perfection in aesthetics and design. So, What Are You Waiting For? Start Using xev0r Today at " .. website})
